...
// Modified by Rook Systems, Inc. in 2026 from rmf_ros2 2.1.8: recorder calls
// at the entry of each inbound callback and before each heartbeat publish.
// The whole change is ../tap.patch.
...
public:
    ...
    recorder(output_directory)
  {
    ...
    blockade_reached_sub =
      ...
      {
        this->recorder.write_reached(*msg);
    ...
    blockade_release_sub =
      ...
      {
        this->recorder.write_release(*msg);
    ...
    blockade_cancel_sub =
      ...
      {
        this->recorder.write_cancel(*msg);
    ...
    heartbeat_timer = create_wall_timer(
      ...
      [this]()
      {
        this->recorder.write_heartbeat_timer();
        this->publish_status();
      });
  }
  ...
  void publish_status()
  {
    ...
    recorder.write_heartbeat(msg);
    heartbeat_pub->publish(msg);
  }

  std::shared_ptr<rmf_traffic::blockade::Moderator> moderator;
  rook_blockade_tap::Recorder recorder;
...
std::shared_ptr<rclcpp::Node> make_node(
  ...
  const rclcpp::NodeOptions& options)
{
  return make_node("rmf_traffic_blockade_node", output_directory, options);
}
...
std::shared_ptr<rclcpp::Node> make_node(
  ...
  const rclcpp::NodeOptions& options)
{
  auto node = std::make_shared<BlockadeNode>(
    node_name, output_directory, options);
